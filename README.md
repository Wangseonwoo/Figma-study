...
## 14. Contextual tool bar
#### 캔버스 혹은 레이어에서 지정 할때마다 달라진다.
#### 아무것도 설정 안했을때 -> 파일의 위치와 파일의 제목을 알려줌
### 파일 제목 옆 탭 중
#### : show version history = 수정한 history를 날짜와 시간대별로 보여줌
### 이미지 클릭시
#### : 세부 수정모드 / 이미지를 컴포넌트로 만들기 / 마스크처리 / 크롭 처리
### 오브젝트 클릭시
#### : 이미지와 같으며 단 크롭처리 기능은 없다.
### 텍스트 클릭시
#### : 이미지와 오브젝트와 앞 3가지 기능은 같으나 링크 기능 추가.
### 프레임 선택시
#### : 컴포넌트 만들기 / 마스크처리 / dev모드 전환
### 프레임 두개 선택 시
#### : 섹션으로 묶기 / (다양한 방식으로)컴포넌트로 만들기 / 마스크 처리 / dev모드 전환
### 컴포넌트 선택 시
#### : 베리언트에 추가 / 마스크 처리 / dev모드 전환
### 섹션 선택 시
#### : dev모드 전환
### 오브젝트 두개 선택 시
#### 불리언 연산 추가
...
## 15. Shared and View Setting
#### 우측 상단 아이콘 -> 현재 어떠한 사용자가 접속했는지 확인 가능

<br><br>

## 16. Asset Panel
#### 디자인을 하기 위한 리소스
#### 현재 파일의 리소스 뿐만 아니라 연관되어 있는 다른 파일들의 리소스도 확인 가능
...
## 18. frame 그리고 다루기
### 프레임 생성
#### : 직접 그릴수도 있지만 프레임 아이콘 클릭 후 우측을 보면 디바이스별 프레임을 맞춤 제작 가능
### 복제
#### : alt + 드래그 or ctrl + D
### 선택
#### : shift + 해당요소 클릭 (왼쪽 패널에 이름을 클릭하는게 편하다.)
...
## 20. 선, 화살표 그리기
#### : 그냥 그리면 시작 지점부터 마우스 포인터까지 선이 그려짐 
### alt + 드래그 = 중앙기준
### shift + 드래그 = 정해진 각도 대로
